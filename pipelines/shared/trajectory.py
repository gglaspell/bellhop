"""
shared/trajectory.py - Shared trajectory-loading and column-normalization
utilities.

REFACTOR NOTE (moved from atlas_pipeline/common):
This module used to live at `pipelines/atlas_pipeline/common/trajectory.py`.
Having a `shared/` package for pipeline-wide helpers AND a second,
texture-baking-only `atlas_pipeline/common/` package a couple of
directories up from it (holding conceptually the same kind of thing --
shared, non-pipeline-specific utility code) was a confusing split with no
real justification, since nothing here is atlas-bake-specific. Its
contents now live directly in `shared/`, alongside `trajectory.py`'s
existing neighbors (`ros_io.py`, `registration.py`, etc.). Every caller in
`atlas_pipeline/` (`keyframeselector.py`, `pointcloudutils.py`,
`meshgenerator.py`, `atlaspacker.py`, `viewassignment.py`,
`visibilityfilter.py`) and in `texture_baking.py` itself has been updated
to import from `shared.trajectory` instead of
`atlas_pipeline.common.trajectory`. `atlas_pipeline/common/` no longer
exists.
