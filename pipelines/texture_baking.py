...
See pointcloud-frame-check-prompt-2.md. This pipeline previously used
every point-cloud frame's points completely unmodified, implicitly
assuming --pc_topic was already published in the same global/fixed frame
as the trajectory built from --odom_topic. There was no check and no way
to run against a raw, non-deskewed sensor-frame cloud.
...
def run(args: argparse.Namespace) -> None:
    ...
    if frame_mode == "local":
        print(
            ...
            "transformed into the world frame via an odometry-derived pose "
            "before frustum-visibility filtering, meshing, and normal "
            "orientation."
        )
    else:
        print(
    ...
    pc_processor.merge_point_clouds_from_bag(
        ...
        frame_mode=frame_mode, odom_max_latency=args.odom_max_latency,
    )
...
def build_parser(sub):
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "global (points used unchanged) and anything else as local "
            "(each frame is transformed into the world frame via an "
            "odom-derived pose before visibility filtering, meshing, and "
            "normal orientation). Use 'global'/'local' to override when a "
            "bag's frame_id is missing, wrong, or empty -- e.g. force "
            "'local' to run directly against a raw, non-deskewed "
    ...
    parser.add_argument(
        ...
        default=0.5,
        help=(
            "Only used when the point cloud is in a local frame (see "
            "--pc_frame_mode): max seconds a point-cloud frame's timestamp "
            "may fall outside trajectory/odometry coverage before it is "
            "dropped."
        ),
    )
